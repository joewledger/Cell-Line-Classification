...
class SVM_Classifier:
	...
	def __init__(self,filename,datatype):
		if(datatype == "Mutation"):
			...
			df  = DataFormatting(filename,datatype)
			self.data_matrix = df.generate_cell_line_expression_matrix(True)
			self.ic_50_dict = df.trim_dict(df.generate_ic_50_dict(),list(data_matrix.columns.values))
