...
def cross_val_score_filter_feature_selection_feature_rank(estimator,num_features, X, y=None, scoring=None, cv=None, n_jobs=1,
                    verbose=0, fit_params=None, score_func=None,
                    pre_dispatch='2*n_jobs'):
    """Evaluate a score by cross-validation
    ...
    scores = parallel(
        delayed(_cross_val_score)(clone(estimator), trim_X_num_features(X,y,train,num_features), y, scorer, train, test,
                                  verbose, fit_params)
        for train, test in cv)
    return np.array(scores)


def cross_val_score_filter_feature_selection_threshold(estimator,threshold, X, y=None, scoring=None, cv=None, n_jobs=1,
                    verbose=0, fit_params=None, score_func=None,
                    pre_dispatch='2*n_jobs'):
    """Evaluate a score by cross-validation
    ...
    scores = parallel(
        delayed(_cross_val_score)(clone(estimator), trim_X_threshold(X,y,train,threshold), y, scorer, train, test,
                                  verbose, fit_params)
        for train, test in cv)
    return np.array(scores)
